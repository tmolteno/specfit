...
def process_bic(outfile, official_name, key, _S, _sigma, _frequency, order):
...
def process(outfile, official_names, key, _S, _sigma, _frequency, order):
    ...
    with h5py.File('calibrator_catalogue.hdf5', 'a') as f:
        ...
        ds_mean = grp.require_dataset("mean", shape=stats[0].shape, dtype=stats[0].dtype, data=stats[0])
        ds_sdev = grp.require_dataset("sdev", shape=stats[1].shape, dtype=stats[1].dtype, data=stats[1])
        ds_cov = grp.require_dataset("cov", shape=a_cov.shape, dtype=a_cov.dtype, data=a_cov)
        ds_cor = grp.require_dataset("corr", shape=a_corr.shape, dtype=a_corr.dtype, data=a_corr)


    sf.full_column(outfile, official_names, idata.posterior, np.array(freq))
    #print(pm.summary(idata))
    fig, ax = sf.dataplot(plt, official_names[0], freq, S, sigma)
    a, da = stats
    nu = np.geomspace(freq[0], freq[-1], 100)
    ax.plot(nu/1e9, sf.flux(nu, a, nu0), label='Polynomial model')

    # We wan't to do in-sample predictions at the new range of frequencies (nu)

    # ppc = sf.posterior_predictive_sampling(idata, mcmc_model, num_pp_samples=2000)
    with mcmc_model:
        # pm.set_data({'frequencies': nu})
        ppc = pm.sample_posterior_predictive(idata,
                                             var_names=['likelihood', 'frequencies']).posterior_predictive
    ...
    fig, ax = sf.posterior_plot(plt, official_names[0], freq, idata, nu0)
...
if __name__ == "__main__":
    ...
    if do_marginal_likelihood:
        ...
        """, file=outfile)
            for calibrator in stars:
                official_name = full_names[calibrator][0]
                mu = data[calibrator]
                sigma = mu*(err / 100)
                process_bic(outfile, official_name, calibrator, mu, sigma, frequency, orders[calibrator])
            print("""
        \\end{tabular}
        \\end{table}
        """, file=outfile)
    ...
    with open('perley_butler_2017_1.tex', 'w') as outfile:
        for calibrator in stars[0:split]:
            ...
            mu = data[calibrator]
            sigma = mu*(err / 100)
            process(outfile, official_name, calibrator, mu, sigma, frequency, orders[calibrator])
